...
export async function md3HasState(
  component: Locator,
  componentType: ComponentName,
  state: string,
): Promise<boolean> {
  ...
  if ('states' in def && def.states) {
    ...
    if (stateSelector) {
      return (
        (await component.locator(`:scope${stateSelector}`).count()) > 0 ||
        (await component
          .filter({ has: component.page().locator(stateSelector) })
          .count()) > 0
      )
    }
  }
...
export async function expectMd3Visible(
  page: Page,
  component: ComponentName,
...
export async function expectMd3Accessible(
  page: Page,
  component: ComponentName,
  options?: { label?: string },
) {
...
export function getToken(
  category: 'colors' | 'elevation' | 'shape',
  token: string,
): string {
...
export function getMotionDuration(
  duration: keyof typeof schema.tokens.motion.duration,
): number {
