...
export const apiClient = {
  async fetchUser(id: string) {
    ...
    return response.json();
        ...
        const secretFindings = secResult.findings.filter(
          f =>
            f.severity === 'critical' ||
...
`
  ...
  return <div>{processData(data)}</div>;
        ...
        const archResult = await archChecker.analyze([
...
export const process = (a, b, c, d) => {
  ...
  return 'none';
        ...
        const securityFindings = secResult.findings.filter(
...
export const database = {
  verify: (user, secret) => authenticate(user)
        ...
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        const secSecrets = secResult.findings.filter(
...
export const LoginForm = ({ onSubmit }) => {
  ...
  return <form onSubmit={handleSubmit}></form>;
        ...
        const secResult = await securityScanner.analyze([
          'src/components/LoginForm.tsx',
        ])
...
export const isValidPhone = (phone) => /^\\d{10}$/.test(phone);
        ...
        const secFinding = secResults.findings.filter(
          f =>
            f.title.toLowerCase().includes('secret') ||
