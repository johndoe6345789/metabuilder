...
describe('QualityValidator - Comprehensive Tests (60+ cases)', () => {
  ...
  describe('Validate Method Orchestration', () => {
    ...
    it('should load configuration', async () => {
      await validator.validate({})
      expect(
        configLoaderModule.configLoader.loadConfiguration,
      ).toHaveBeenCalled()
    })
    ...
    it('should handle show profile option', async () => {
      const mockGetProfile = jest
        .spyOn(profileManagerModule.profileManager, 'getProfile')
        .mockReturnValue({
          name: 'test',
          description: 'test',
          weights: {
    ...
    it('should handle configuration error', async () => {
      const mockLoadConfig = jest
        .spyOn(configLoaderModule.configLoader, 'loadConfiguration')
        .mockRejectedValue(new SyntaxError('Invalid config'))
    ...
    it('should handle analysis errors', async () => {
      const mockAnalyze = jest
        .spyOn(codeQualityModule.codeQualityAnalyzer, 'analyze')
  ...
  describe('Validate Method with Various Configurations', () => {
    ...
    it('should handle skip coverage option', async () => {
      const mockApplyCliOptions = jest
        .spyOn(configLoaderModule.configLoader, 'applyCliOptions')
        .mockImplementation((config, options) => {
    ...
    it('should warn when no source files found', async () => {
      const mockGetSourceFiles = jest
        .spyOn(fileSystemModule, 'getSourceFiles')
        .mockReturnValue([])
    ...
    it('should return fail status for low scores', async () => {
      const mockCalculateScore = jest
        .spyOn(scoringModule.scoringEngine, 'calculateScore')
        .mockReturnValue({
          overall: {
            score: 45,
            grade: 'F',
  ...
  describe('Result Generation and Reporting', () => {
    ...
    it('should include recommendations in result', async () => {
      await validator.validate({})
      const mockCalculate = jest.spyOn(
        scoringModule.scoringEngine,
        'calculateScore',
      )
  ...
  describe('Error Handling', () => {
    ...
    it('should handle analyzer failures gracefully', async () => {
      const mockAnalyze = jest
        .spyOn(codeQualityModule.codeQualityAnalyzer, 'analyze')
        .mockRejectedValue(new Error('Analyzer error'))
    ...
    it('should catch and report exceptions', async () => {
      const mockInitialize = jest
        .spyOn(profileManagerModule.profileManager, 'initialize')
  ...
  describe('Analyzer Orchestration', () => {
    ...
    it('should pass source files to analyzers', async () => {
      await validator.validate({})
      expect(
        codeQualityModule.codeQualityAnalyzer.analyze,
      ).toHaveBeenCalledWith(['src/test.ts'])
    })
  })
})
