...
describe('DBALStorageAdapter', () => {
  ...
  describe('deleteSnippet', () => {
    ...
    it('should throw error on failed deletion', async () => {
      ...
      const adapter = new DBALStorageAdapter(baseUrl)
      await expect(adapter.deleteSnippet('nonexistent')).rejects.toThrow(
  ...
  describe('HTTP Error Responses', () => {
    ...
    it('should handle 500 Internal Server Error', async () => {
      ...
      await expect(adapter.updateSnippet(createMockSnippet())).rejects.toThrow(
        'Failed to update snippet',
      )
    })
  ...
  describe('Edge Cases and Boundary Conditions', () => {
    ...
    it('should handle URL with trailing slashes correctly', async () => {
      const adapterWithTrailingSlash = new DBALStorageAdapter(
        'http://localhost:5000/',
      )
      ;(global.fetch as jest.Mock).mockResolvedValue({
        ok: true,
