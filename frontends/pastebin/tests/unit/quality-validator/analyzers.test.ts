...
describe('Code Quality Analyzer', () => {
  ...
  describe('Code Duplication Detection', () => {
    ...
    it('should identify affected files', () => {
      const blocks = [
        {
          locations: [{ file: 'file1.ts' }, { file: 'file2.ts' }],
          size: 100,
          lines: [],
          suggestion: '',
        },
        {
          locations: [{ file: 'file2.ts' }, { file: 'file3.ts' }],
          size: 80,
          lines: [],
          suggestion: '',
        },
        {
          locations: [{ file: 'file1.ts' }, { file: 'file4.ts' }],
          size: 60,
          lines: [],
          suggestion: '',
        },
    ...
    it('should calculate total duplicated lines', () => {
      ...
      const totalDuplicatedLines = blocks.reduce(
  ...
  describe('Component Size Analysis', () => {
    ...
    it('should calculate average by type', () => {
      ...
      const atomAverage =
        atoms.reduce((sum, c) => sum + c.lines, 0) / atoms.length
      expect(atomAverage).toBe(50)

      const molecules = components.filter(c => c.type === 'molecule')
      const moleculeAverage =
        molecules.reduce((sum, c) => sum + c.lines, 0) / molecules.length
    ...
    it('should score ineffective tests', () => {
      const effectiveness = {
        ...
        issues: [
          {
            file: 'test.ts',
            issue: 'Tests with too many mocks',
            suggestion: 'Reduce mock usage',
            severity: 'high',
          },
        ],
      }
...
describe('Architecture Checker', () => {
  describe('Component Organization', () => {
    ...
    it('should track misplaced components', () => {
          ...
          misplaced: [
            {
              ...
              suggestedLocation: 'src/components/atoms',
            },
            {
              file: 'Form.tsx',
              name: 'Form',
              currentLocation: 'src/components',
              suggestedLocation: 'src/components/molecules',
            },
          ],
          averageSize: 200,
        },
      ...
      expect(metrics.components.misplaced[0].suggestedLocation).toContain(
        'atoms',
      )
    })
  })
  ...
  describe('Dependency Analysis', () => {
    ...
    it('should detect circular dependencies', () => {
      const cycles: CircularDependency[] = [
        ...
        {
          path: ['ComponentC', 'ComponentD', 'ComponentE', 'ComponentC'],
    ...
    it('should identify deeply nested dependencies', () => {
      ...
      const getDepth = (
        ...
        return (
          1 + Math.max(...children.map(c => getDepth(c, graph, visited)), 0)
        )
      }
  ...
  describe('Layer Violations', () => {
    ...
    it('should detect cross-layer violations', () => {
      const violations = [
        {
          source: 'data/repo.ts',
          target: 'presentation/component.tsx',
          violationType: 'direct-access',
          suggestion: 'Use business layer',
        },
      ]
    ...
    it('should validate architecture compliance', () => {
      ...
      const isValid = (source: string, target: string): boolean => {
        const sourceLayer = source.split('/')[0] as
          | 'presentation'
          | 'business'
          | 'data'
        const allowed =
          rules[`${sourceLayer}-can-use` as keyof typeof rules] || []
...
describe('Security Scanner', () => {
  ...
  describe('Secret Detection', () => {
    ...
    it('should scan multiple secret patterns', () => {
      ...
      const found = secrets.filter(
        s => s.includes('.env') || s.includes('secret'),
      )
  ...
  describe('Dependency Vulnerability Scanning', () => {
    ...
    it('should track affected packages', () => {
      const packages = ['lodash@4.17.19', 'express@4.16.0', 'axios@0.18.0']
