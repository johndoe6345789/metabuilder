...
describe('Quality Validator Orchestrator', () => {
  ...
  describe('Analyzer configuration', () => {
    it('should configure complexity analysis', () => {
      ...
      expect(config.codeQuality.complexity.warning).toBeLessThan(
    ...
    it('should configure coverage thresholds', () => {
      ...
      expect(config.testCoverage.warningPercent).toBeLessThan(
        config.testCoverage.minimumPercent,
      )
    })
    ...
    it('should configure security limits', () => {
      const config = createMockConfig()
      expect(
        config.security.vulnerabilities.allowCritical,
      ).toBeGreaterThanOrEqual(0)
      expect(config.security.vulnerabilities.allowHigh).toBeGreaterThanOrEqual(
        0,
      )
    })
  })
  ...
  describe('Analysis workflow', () => {
    ...
    it('should aggregate findings from all analyzers', () => {
      const findings = [
        ...
        },
        {
          ...
          location: { file: '.env.example', line: 1 },
    ...
    it('should track parallel execution of analyzers', () => {
      ...
      const totalSequential = Object.values(executionTimes).reduce(
        (a, b) => a + b,
        0,
      )
  ...
  describe('Findings collection', () => {
    ...
    it('should collect findings by category', () => {
      const findingsByCategory = {
        ...
        testCoverage: [{ id: 'tc-001', severity: 'medium', title: 'Issue 3' }],
        architecture: [{ id: 'arch-001', severity: 'high', title: 'Issue 4' }],
        security: [{ id: 'sec-001', severity: 'critical', title: 'Issue 5' }],
      }
  ...
  describe('Recommendations generation', () => {
    ...
    it('should prioritize critical recommendations', () => {
      ...
      const criticalOnly = recommendations.filter(
  ...
  describe('Exit codes', () => {
    it('should return success for passing quality', () => {
      ...
      const exitCode =
        overallStatus === 'pass' ? ExitCode.SUCCESS : ExitCode.QUALITY_FAILURE
