...
function createMockComponentScores(
  codeQuality = 85,
  testCoverage = 90,
  architecture = 75,
  security = 88,
  ...
  return {
    codeQuality: {
      score: codeQuality,
      weight: 0.25,
      weightedScore: codeQuality * 0.25,
    },
    testCoverage: {
      score: testCoverage,
      weight: 0.25,
      weightedScore: testCoverage * 0.25,
    },
    architecture: {
      score: architecture,
      weight: 0.25,
      weightedScore: architecture * 0.25,
    },
...
describe('TrendStorage', () => {
  ...
  describe('loadTrendHistory', () => {
    ...
    it('should load existing history from file', () => {
      const record1 = createHistoricalRecord(
        85,
        'B',
        createMockComponentScores(),
      )
  ...
  describe('saveTrendHistory', () => {
    it('should save a new record', () => {
      const record = createHistoricalRecord(
        90,
        'A',
        createMockComponentScores(),
      )
    ...
    it('should maintain rolling window of max 30 records', () => {
      ...
      for (let i = 0; i < 35; i++) {
        ...
        const record = createHistoricalRecord(
          score,
          'B',
          createMockComponentScores(score),
        )
  ...
  describe('getLastRecord', () => {
    ...
    it('should return the most recent record', () => {
      ...
      const record2 = createHistoricalRecord(
        85,
        'B',
        createMockComponentScores(),
      )
  ...
  describe('getRecordsForDays', () => {
    it('should filter records by date range', () => {
      ...
      const oldRecord = createHistoricalRecord(
        80,
        'B',
        createMockComponentScores(),
      )
      oldRecord.timestamp = new Date(
        now - 10 * 24 * 60 * 60 * 1000,
      ).toISOString() // 10 days ago

      const recentRecord = createHistoricalRecord(
        85,
        'B',
        createMockComponentScores(),
      )
      recentRecord.timestamp = new Date(
        now - 1 * 24 * 60 * 60 * 1000,
  ...
  describe('getTrendRecommendation', () => {
    ...
    it('should provide recommendation for concerning metrics', () => {
      ...
      expect(recommendation?.toLowerCase()).toMatch(
        /declining|review|momentum|focus/,
      )
    })
  })
  ...
  describe('edge cases', () => {
    ...
    it('should handle rapid score changes', () => {
      const record = createHistoricalRecord(
        50,
        'F',
        createMockComponentScores(50, 50, 50, 50),
      )
      saveTrendHistory(record)
