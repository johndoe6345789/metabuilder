...
jest.mock('@/components/settings/SchemaHealthCard', () => ({
  SchemaHealthCard: () => (
    <div data-testid="schema-health-card">Schema Health</div>
  ),
}))

jest.mock('@/components/settings/BackendAutoConfigCard', () => ({
  BackendAutoConfigCard: () => (
    <div data-testid="backend-auto-config-card">Backend Config</div>
  ),
}))

jest.mock('@/components/settings/StorageBackendCard', () => ({
  StorageBackendCard: () => (
    <div data-testid="storage-backend-card">Storage Backend</div>
  ),
}))

jest.mock('@/components/settings/DatabaseStatsCard', () => ({
  DatabaseStatsCard: () => (
    <div data-testid="database-stats-card">Database Stats</div>
  ),
}))

jest.mock('@/components/settings/StorageInfoCard', () => ({
  StorageInfoCard: () => (
    <div data-testid="storage-info-card">Storage Info</div>
  ),
}))

jest.mock('@/components/settings/DatabaseActionsCard', () => ({
  DatabaseActionsCard: () => (
    <div data-testid="database-actions-card">Database Actions</div>
  ),
}))

jest.mock('@/components/settings/OpenAISettingsCard', () => ({
  OpenAISettingsCard: () => (
    <div data-testid="openai-settings-card">OpenAI Settings</div>
  ),
}))
...
jest.mock('@/components/atoms/AtomsSection', () => ({
  AtomsSection: ({ onSaveSnippet }: any) => (
    <div
      ...
      onClick={() => onSaveSnippet({ title: 'test' })}
    >
      Atoms Section
    </div>
  ),
...
describe('App Pages', () => {
  describe('Settings Page', () => {
    ...
    it('should render settings description', async () => {
      ...
      expect(
        screen.getByText('Manage your database and application settings'),
      ).toBeInTheDocument()
    })
