...
jest.mock('./FormFieldsSection', () => ({
  ...
  FormFieldsSection: ({ onSaveSnippet }: any) => (
    <div data-testid="form-fields-section">Form Fields</div>
  ),
}))
...
jest.mock('./SearchBarsSection', () => ({
  ...
  SearchBarsSection: ({ onSaveSnippet }: any) => (
    <div data-testid="search-bars-section">Search Bars</div>
  ),
}))

jest.mock('./UserCardsSection', () => ({
  UserCardsSection: () => (
    <div data-testid="user-cards-section">User Cards</div>
  ),
}))

jest.mock('./SocialActionsSection', () => ({
  SocialActionsSection: () => (
    <div data-testid="social-actions-section">Social Actions</div>
  ),
}))

jest.mock('./StatusIndicatorsSection', () => ({
  StatusIndicatorsSection: () => (
    <div data-testid="status-indicators-section">Status Indicators</div>
  ),
}))

jest.mock('./ContentPreviewCardsSection', () => ({
  ContentPreviewCardsSection: () => (
    <div data-testid="content-preview-cards-section">Content Preview</div>
  ),
}))
