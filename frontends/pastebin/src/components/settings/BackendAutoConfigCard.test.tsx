...
describe('BackendAutoConfigCard', () => {
  ...
  describe('rendering when not configured', () => {
    it('should render nothing when envVarSet is false', () => {
      const { container } = render(<BackendAutoConfigCard envVarSet={false} />)
  ...
  describe('rendering when configured', () => {
    it('should render card when envVarSet is true', () => {
      render(<BackendAutoConfigCard envVarSet={true} />)

      expect(screen.getByText('Backend Auto-Configured')).toBeInTheDocument()
    })
    ...
    it('should have accent styling when configured', () => {
      const { container } = render(<BackendAutoConfigCard envVarSet={true} />)

      const card = container.querySelector(
        '[data-testid="backend-auto-config-card"]',
      )
  ...
  describe('conditional rendering', () => {
    it('should only render when envVarSet changes from false to true', () => {
      const { rerender, container } = render(
        <BackendAutoConfigCard envVarSet={false} />,
      )

      expect(container.firstChild).toBeNull()

      rerender(<BackendAutoConfigCard envVarSet={true} />)

      expect(screen.getByText('Backend Auto-Configured')).toBeInTheDocument()
    })

    it('should maintain rendered state when envVarSet stays true', () => {
      const { rerender } = render(<BackendAutoConfigCard envVarSet={true} />)
