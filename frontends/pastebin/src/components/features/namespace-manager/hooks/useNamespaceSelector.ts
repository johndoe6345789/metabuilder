...
import { createNamespace, deleteNamespace } from '@/store/slices/namespacesSlice'
...
export function useNamespaceSelector({
  ...
  const handleDeleteNamespace = async () => {
    ...
    setLoading(true)
    try {
      const snippetsToMove = await getSnippetsByNamespace(
        namespaceToDelete.id,
      )
      if (snippetsToMove.length > 0) {
        await bulkMoveSnippets(
          snippetsToMove.map(s => s.id), defaultNamespace.id,
        )
      }
  ...
  return {
    namespaces,
    newNamespaceName, createDialogOpen, deleteDialogOpen,
    namespaceToDelete, loading,
    ...rename,
    setNewNamespaceName, setCreateDialogOpen,
    setDeleteDialogOpen, setNamespaceToDelete,
