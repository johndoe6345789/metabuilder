...
jest.mock('@metabuilder/components/fakemui', () => ({
  Button: ({ children, onClick, variant, 'data-testid': testId }: any) => (
    <button onClick={onClick} data-variant={variant} data-testid={testId}>{children}</button>
  ),
  Dialog: ({ open, onClose, children }: any) =>
    open ? <div role="dialog" data-testid="delete-dialog">{children}</div> : null,
...
describe('SnippetDeleteDialog', () => {
  ...
  it('renders snippet title in body text', () => {
    render(<SnippetDeleteDialog {...defaultProps} />)
    expect(screen.getByText(/"My Snippet" will be permanently deleted./)).toBeInTheDocument()
  })
  ...
  it('renders delete label on confirm button', () => {
    render(<SnippetDeleteDialog {...defaultProps} />)
    expect(screen.getByTestId('confirm-delete-snippet-btn').textContent).toBe('Delete')
  })

  it('uses the snippetTitle in the body correctly', () => {
    render(<SnippetDeleteDialog {...defaultProps} snippetTitle="special & snippet" />)
    expect(screen.getByText(/"special & snippet" will be permanently deleted./)).toBeInTheDocument()
  })
})
