...
import {
  fetchRevisions, revertToRevision, forkSnippet, forkSharedSnippet,
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchRevisions.fulfilled, (state, action) => {
        state.loading = false
        state.bySnippetId[action.payload.snippetId] =
      ...
      .addCase(revertToRevision.rejected, (state, action) => {
        state.loading = false
        state.error =
          action.error.message || 'Failed to revert to revision'
      })
      .addCase(forkSnippet.pending, pendingReducer)
      .addCase(forkSnippet.fulfilled, state => { state.loading = false })
      ...
      .addCase(forkSharedSnippet.rejected, (state, action) => {
        state.loading = false
        state.error =
          action.error.message || 'Failed to fork shared snippet'
      })
      .addMatcher(
