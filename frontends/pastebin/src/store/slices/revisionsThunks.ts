...
import {
  entityUrl, authHeaders, getUserId,
  toRevision, toSnippet, buildForkBody,
} from './revisions-dbal'
...
export const revertToRevision = createAsyncThunk(
  'revisions/revert',
  async (
    { snippetId, revisionId }: { snippetId: string; revisionId: string }
  ) => {
    ...
    const body: Record<string, unknown> = {
      code: revision.code, updatedAt: Date.now(),
    }
