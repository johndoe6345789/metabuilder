...
import type { ParallelAnalyzerTask, ParallelExecutionResult } from './ParallelAnalyzer.js';
...
export function estimateTime(
  ...
  const serial =
    fileCount * timePerFile * analyzerCount +
    analyzerOverhead * analyzerCount;
  const parallel =
    fileCount * timePerFile +
    (analyzerOverhead * analyzerCount) /
      Math.min(analyzerCount, workerCount);
