...
export interface ParallelExecutionResult {
  ...
  totalTime: number; parallelEfficiency: number; parallelRatio: number;
}
...
export class ParallelAnalyzer {
  ...
  constructor(o: { workerCount?: number; fileChunkSize?: number; onProgress?: ProgressCallback } = {}) {
  ...
  async processWithAnalyzer(
    a: ParallelAnalyzerTask, files: string[]
  ): Promise<AnalysisResult | null> {
    if (!a.enabled) { logger.debug(`Disabled: ${a.name}`); return null; }
    try {
  ...
  async runParallel(
    analyzers: ParallelAnalyzerTask[], files: string[]
  ): Promise<ParallelExecutionResult> {
    const t0 = performance.now();
    const enabled = analyzers.filter(a => a.enabled);
    if (!enabled.length) {
      logger.warn('No analyzers enabled');
      return { results: analyzers.map(() => null), totalTime: 0, parallelEfficiency: 0, parallelRatio: 0 };
    }
    ...
    return { results: all, totalTime, parallelEfficiency: 100, parallelRatio: 1 };
  }
  async runChunked(
    a: ParallelAnalyzerTask, files: string[]
  ): Promise<AnalysisResult | null> {
    return runChunkedAnalysis(
      a, files, this.fileChunkSize, this.progressCallback,
      this.processWithAnalyzer.bind(this)
    );
  }
  async runBalanced(
    analyzers: ParallelAnalyzerTask[], files: string[], max = 4
  ): Promise<ParallelExecutionResult> {
    return runBalancedAnalysis(
      analyzers, files, max, this.processWithAnalyzer.bind(this)
    );
  }
...
export async function executeAnalyzersParallel(
  ...
  return (await new ParallelAnalyzer({ workerCount: o.workerCount || 4, onProgress: o.onProgress }).runParallel(analyzers, files)).results;
}
