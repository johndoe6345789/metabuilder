...
import {
  Configuration, ConfigurationError, CommandLineOptions,
} from '../types/index.js';
...
export class ConfigLoader {
  ...
  async loadConfiguration(configPath?: string): Promise<Configuration> {
    ...
    if (configPath) {
      ...
      for (const loc of ['.qualityrc.json', '.quality/config.json']) {
        if (fs.existsSync(loc)) { file = this.loadConfigFile(loc); break; }
      }
    }
  ...
  private loadConfigFile(filePath: string): Partial<Configuration> {
    try {
      ...
      if (typeof cfg !== 'object' || cfg === null) {
        throw new ConfigurationError(
          'Configuration must be a JSON object', `Got: ${typeof cfg}`
        );
      }
      ...
      if (e instanceof SyntaxError) {
        throw new ConfigurationError(
          `Invalid JSON: ${filePath}`, (e as Error).message
        );
      }
      throw new ConfigurationError(
        `Failed to read: ${filePath}`, (e as Error).message
      );
    }
  }

  applyCliOptions(
    config: Configuration, options: CommandLineOptions
  ): Configuration {
  ...
  getDefaults = () => JSON.parse(JSON.stringify(DEFAULT_CONFIG)) as Configuration;
  getMinimalConfig = () => JSON.parse(JSON.stringify(DEFAULT_CONFIG)) as Configuration;
}
