...
export function loadProfilesFromFile(filePath) {
    ...
    try {
        ...
        if (typeof data !== 'object' || data === null) {
            throw new ConfigurationError(`Invalid profiles file: ${filePath}`, 'Profiles must be a JSON object');
        }
        return data;
    }
    catch (err) {
        ...
        if (err instanceof SyntaxError) {
            throw new ConfigurationError(`Invalid JSON in profiles file: ${filePath}`, err.message);
        }
        throw new ConfigurationError(`Failed to read profiles file: ${filePath}`, err.message);
    }
}
