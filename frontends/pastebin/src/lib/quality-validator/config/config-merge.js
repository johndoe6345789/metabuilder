...
export function validateConfigWeightsAndThresholds(config) {
    const w = config.scoring.weights;
    const sum = w.codeQuality + w.testCoverage + w.architecture + w.security;
    if (Math.abs(sum - 1.0) > 0.001) {
        throw new ConfigurationError('Scoring weights must sum to 1.0', `Got: ${sum.toFixed(4)}. Weights: ${JSON.stringify(w)}`);
    }
    if (config.testCoverage.minimumPercent < 0 ||
    ...
    if (c.warning > c.max) {
        throw new ConfigurationError('Complexity warning must be less than max', `Warning: ${c.warning}, Max: ${c.max}`);
    }
    const d = config.codeQuality.duplication;
    if (d.warningPercent > d.maxPercent) {
        throw new ConfigurationError('Duplication warning must be less than max', `Warning: ${d.warningPercent}%, Max: ${d.maxPercent}%`);
    }
    if (!['A', 'B', 'C', 'D', 'F'].includes(config.scoring.passingGrade)) {
        throw new ConfigurationError('Invalid passing grade', `Got: ${config.scoring.passingGrade}`);
    }
}
