...
export class ArchitectureChecker extends BaseAnalyzer {
    ...
    generateFindings(metrics) {
        for (const component of metrics.components.oversized.slice(0, 3)) {
            this.addFinding({
                ...
                description: `Component '${component.name}' has ${component.lines} lines, ` +
    ...
    calculateScore(metrics) {
        ...
        const dependencyScore = Math.max(0, 100 - dependencies.circularDependencies.length * 20);
        const patternScore = (patterns.reduxCompliance.score +
