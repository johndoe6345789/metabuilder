...
export class QualityValidator {
  ...
  async validate(options: CommandLineOptions = {}): Promise<number> {
    try {
      ...
      if (options.listProfiles) return handleListProfiles()
      ...
      const scoringResult = scoringEngine.calculateScore(
        cqResult?.metrics as any,
        tcResult?.metrics as any,
        archResult?.metrics as any,
        secResult?.metrics as any,
      ...
      logger.info(
        `Quality validation ${scoringResult.overall.status}: ` +
