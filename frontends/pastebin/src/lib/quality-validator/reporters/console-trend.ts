...
export function renderTrendSection(
  ...
  if (trend.previousScore !== undefined) {
    ...
    const sign = change >= 0 ? '+' : ''
    const sym =
      change > 0 ? '↑ improving' : change < 0 ? '↓ degrading' : '→ stable'
  ...
  if (typeof td.sevenDayAverage === 'number' && td.sevenDayAverage > 0) {
    ...
    const diff = (trend.currentScore - (td.sevenDayAverage as number)).toFixed(
      1,
    )
  ...
  if (typeof td.thirtyDayAverage === 'number' && td.thirtyDayAverage > 0) {
    ...
    const diff = (trend.currentScore - (td.thirtyDayAverage as number)).toFixed(
      1,
    )
    const sign = Number(diff) >= 0 ? '+' : ''
  ...
  if (typeof td.volatility === 'number') {
    ...
    const label =
      v < 1 ? 'Excellent' : v < 3 ? 'Good' : v < 5 ? 'Moderate' : 'High'
    ...
    for (const cat of [
      ...
      const ct = (
        trend.componentTrends as Record<
          string,
          { direction?: string; current: number; change?: number }
        >
      )[cat]
      if (!ct) continue
      const arrow =
