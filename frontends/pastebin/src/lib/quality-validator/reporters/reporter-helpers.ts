...
export function fmtMetadata(
  ...
  return {
    ...
    projectName:
      metadata.configUsed.projectName || 'snippet-pastebin',
  };
}
...
export function fmtFindingStats(
  findings: Finding[]
...
export function fmtTopRecommendations(
  ...
  return [...recs]
    .sort(
      (a, b) => (order[a.priority] ?? 999) - (order[b.priority] ?? 999)
    )
    .slice(0, limit);
}

export function fmtTopFindings(
  findings: Finding[],
  limit = 10
): Finding[] {
  const order: Record<string, number> = {
  ...
  return [...findings]
    .sort(
      (a, b) => (order[a.severity] ?? 999) - (order[b.severity] ?? 999)
    )
    .slice(0, limit);
}
