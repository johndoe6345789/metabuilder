...
function generateCodeQualityMetrics(config: any): string {
  ...
  return metricCard('Code Quality Configuration',
...
function generateTestCoverageMetrics(config: any): string {
  ...
  return metricCard('Test Coverage Configuration',
...
function generateArchitectureMetrics(config: any): string {
  ...
  return metricCard('Architecture Configuration',
...
function generateSecurityMetrics(config: any): string {
  ...
  return metricCard('Security Configuration',
    metricRow(
      'Allow Critical Vulns', sec.vulnerabilities?.allowCritical || 0
    ) +
    metricRow('Allow High Vulns', sec.vulnerabilities?.allowHigh || 0) +
    metricRow(
      'Check Secrets',
      sec.patterns?.checkSecrets ? 'Yes' : 'No'
    ) +
    metricRow(
      'Check XSS Risks',
      sec.patterns?.checkXssRisks ? 'Yes' : 'No'
    )
  );
}
