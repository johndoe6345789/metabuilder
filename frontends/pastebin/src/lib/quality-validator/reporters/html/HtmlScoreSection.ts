...
function gradeClass(g: string): string {
  if (g === 'A' || g === 'B') return 'grade-pass'
  if (g === 'C' || g === 'D') return 'grade-warning'
  return 'grade-fail'
}
...
export function generateOverallSection(overall: OverallScore): string {
  return (
    `<section class="section"><div class="card overall-card">` +
...
export function generateScoreCard(
  ...
  return (
    `<div class="card score-card ${scoreClass(score)}">` +
    `<h3>${name}</h3>` +
    `<div class="score-bar">` +
...
export function generateComponentScoresSection(cs: ComponentScores): string {
  const cards = [
    generateScoreCard(
      'Code Quality',
      cs.codeQuality.score,
      cs.codeQuality.weight,
    ),
    generateScoreCard(
      'Test Coverage',
      cs.testCoverage.score,
      cs.testCoverage.weight,
    ),
    generateScoreCard(
      'Architecture',
      cs.architecture.score,
      cs.architecture.weight,
    ),
    generateScoreCard('Security', cs.security.score, cs.security.weight),
  ].join('')
  return (
    `<section class="section"><h2>Component Scores</h2>` +
