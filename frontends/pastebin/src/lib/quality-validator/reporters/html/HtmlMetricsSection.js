...
function generateCodeQualityMetrics(config) {
    ...
    return metricCard('Code Quality Configuration', metricRow('Complexity Threshold', cq.complexity?.max || 'N/A') +
        metricRow('Complexity Warning', cq.complexity?.warning || 'N/A') +
        metricRow('Max Duplication %', `${cq.duplication?.maxPercent || 'N/A'}%`) +
...
function generateTestCoverageMetrics(config) {
    ...
    return metricCard('Test Coverage Configuration', metricRow('Minimum Coverage', `${tc.minimumPercent || 'N/A'}%`) +
...
function generateArchitectureMetrics(config) {
    ...
    return metricCard('Architecture Configuration', metricRow('Max Component Lines', arch.components?.maxLines || 'N/A') +
        metricRow('Component Warning', arch.components?.warningLines || 'N/A') +
        metricRow('Atomic Design Check', arch.components?.validateAtomicDesign ? 'Enabled' : 'Disabled') +
        metricRow('Pattern Check', arch.patterns?.enabled ? 'Enabled' : 'Disabled'));
}
function generateSecurityMetrics(config) {
    if (!config.security?.enabled)
        return '';
    const sec = config.security;
    return metricCard('Security Configuration', metricRow('Allow Critical Vulns', sec.vulnerabilities?.allowCritical || 0) +
...
export function generateMetricsSection(result) {
    ...
    return (`<section class="section"><h2>Detailed Metrics</h2>` +
