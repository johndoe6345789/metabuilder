...
export class ConsoleReporter extends ReporterBase {
  ...
  private renderHeader(result: ScoringResult, c: Colorizer): string {
    ...
    return [
      ...
      c(`║   ${result.metadata.timestamp.substring(0, 19).padEnd(49)}║`, 'cyan'),
      c(B.bot, 'cyan'),
      '',
    ].join('\n');
  }
  ...
  private renderComponents(result: ScoringResult, c: Colorizer): string {
    ...
    const scores = [
      { name: 'Code Quality',  score: cs.codeQuality.score,  weight: cs.codeQuality.weight },
      { name: 'Test Coverage', score: cs.testCoverage.score, weight: cs.testCoverage.weight },
      { name: 'Architecture',  score: cs.architecture.score, weight: cs.architecture.weight },
      { name: 'Security',      score: cs.security.score,     weight: cs.security.weight },
    ];
  ...
  private renderFindings(findings: unknown, c: Colorizer): string {
    const grouped = this.formatFindingsForDisplay(
      findings as Parameters<typeof this.formatFindingsForDisplay>[0], 3
    );
    ...
    for (const sev of ['critical', 'high', 'medium', 'low', 'info']) {
      const grp = (grouped as Record<string, {
        count: number;
        displayed: { title: string; location?: { file: string; line?: number } }[];
        remaining: number;
      }>)[sev];
      if (!grp) continue;
  ...
  private renderRecommendations(
    recommendations: unknown, c: Colorizer
  ): string {
    const recs = this.getTopRecommendations(
      recommendations as Parameters<typeof this.getTopRecommendations>[0], 5
    );
