...
function makeColorizer(useColors) {
    if (!useColors)
        return t => t;
    const colors = {
        red: '\x1b[31m',
    ...
    return (text, color) => color ? `${colors[color] ?? ''}${text}\x1b[0m` : text;
}
export class ConsoleReporter extends ReporterBase {
    ...
    renderOverall(result, c) {
        const { overall } = result;
        const gc = this.getGradeColor(overall.grade);
        const statusStr = overall.status === 'pass' ? c('✓ PASS', 'green') : c('✗ FAIL', 'red');
        const scorePart = `${c(overall.grade, gc).padEnd(6)} ` +
    ...
    renderFindings(findings, c) {
        ...
        for (const sev of ['critical', 'high', 'medium', 'low', 'info']) {
            ...
            if (grp.remaining > 0)
                lines.push(`│   ... and ${grp.remaining} more`);
        }
        lines.push(c(B.end, 'cyan'), '');
        return lines.join('\n');
    }
    renderRecommendations(recommendations, c) {
        const recs = this.getTopRecommendations(recommendations, 5);
