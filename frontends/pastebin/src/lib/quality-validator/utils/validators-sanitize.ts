...
export function sanitizeFinding(finding: Finding): Finding {
  ...
  return {
    ...finding,
    severity: (
      validSev.includes(finding.severity) ? finding.severity : 'medium'
    ) as Finding['severity'],
    title: finding.title?.trim() || 'Unknown Issue',
    description: finding.description?.trim() || '',
    remediation:
      finding.remediation?.trim() || 'No remediation provided',
...
export function sanitizeRecommendation(
  ...
  return {
    ...rec,
    priority: (
      validPri.includes(rec.priority) ? rec.priority : 'medium'
    ) as Recommendation['priority'],
    issue: rec.issue?.trim() || 'Unknown Issue',
    remediation: rec.remediation?.trim() || '',
    estimatedEffort: (
      validEff.includes(rec.estimatedEffort)
        ? rec.estimatedEffort
        : 'medium'
    ) as Recommendation['estimatedEffort'],
