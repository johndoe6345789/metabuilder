...
export function calcTrend(history) {
    ...
    const direction = changePercent < -5
        ? 'improving'
        : changePercent > 5
            ? 'degrading'
            : 'stable';
...
export function calcAverageMetrics(history) {
    if (history.length === 0) {
        return {
            avgTime: 0,
            avgFileCount: 0,
    ...
    return {
        ...
        avgParallelEfficiency: history.reduce((s, r) => s + r.parallelEfficiency, 0) / n,
    };
}
