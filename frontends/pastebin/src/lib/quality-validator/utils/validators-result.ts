...
import { validateScore, validateFinding, validateRecommendation } from './validators-core.js';
...
export function validateConfiguration(config: Configuration): string[] {
  const errors: string[] = [];
  if (!config) { errors.push('Configuration is required'); return errors; }
  if (config.scoring?.weights) {
    ...
    for (const key of ['codeQuality', 'testCoverage', 'architecture', 'security'] as const) {
      if (w[key] < 0 || w[key] > 1) {
