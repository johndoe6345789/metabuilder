...
export function formatPerformanceReport(report: PerformanceReport): string {
  ...
  output +=
    `Avg Retrieval: ${report.cache.avgRetrievalTime.toFixed(2)}ms\n`;
  ...
  output +=
    `Detection Time: ${report.changeDetection.detectionTime.toFixed(2)}ms\n`;
  ...
  output += '\n--- Metrics ---\n';
  output +=
    `Avg Time/File: ${report.avgTimePerFile.toFixed(2)}ms\n`;
  output +=
    `Status: ${report.thresholdExceeded ? 'EXCEEDED THRESHOLD' : 'OK'}\n`;
