 ...
 * Extended tests for configuration validation, data validation, rule
...
describe('Validators - File Location Validation', () => {
  describe('validateFileLocation', () => {
    ...
    it('should reject negative line numbers', () => {
      ...
      expect(validateFileLocation({ file: 'src/app.ts', line: -100 })).toBe(
        false,
      )
    })

    it('should reject negative column numbers', () => {
      expect(validateFileLocation({ file: 'src/app.ts', column: -1 })).toBe(
        false,
      )
    })
    ...
    it('should reject non-number line/column', () => {
      expect(
        validateFileLocation({ file: 'src/app.ts', line: 'abc' as any }),
      ).toBe(false)
      expect(
        validateFileLocation({ file: 'src/app.ts', column: 'xyz' as any }),
      ).toBe(false)
    })
  })
})
  ...
  describe('validateRecommendation', () => {
    ...
    it('should accept all valid effort levels', () => {
      ...
      for (const effort of efforts) {
        ...
        const errors = validateRecommendation(rec)
        expect(errors.filter(e => e.includes('estimatedEffort'))).toHaveLength(
          0,
        )
      }
    })
...
describe('Validators - File Exclusion', () => {
  describe('shouldExcludeFile', () => {
    it('should exclude node_modules files', () => {
      expect(
        shouldExcludeFile('node_modules/package/index.js', ['node_modules']),
      ).toBe(true)
    })
    ...
    it('should not exclude non-matching files', () => {
      expect(shouldExcludeFile('src/app.ts', ['node_modules', 'dist'])).toBe(
        false,
      )
      expect(
        shouldExcludeFile('tests/app.test.ts', ['node_modules', 'dist']),
      ).toBe(false)
    })
    ...
    it('should handle partial path matching', () => {
      expect(
        shouldExcludeFile('src/node_modules/file.js', ['node_modules']),
      ).toBe(true)
    })
  })
})
