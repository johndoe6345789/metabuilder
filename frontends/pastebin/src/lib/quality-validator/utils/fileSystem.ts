...
export {
  getProjectRoot, resolvePath, normalizeFilePath,
  pathExists, isFile, isDirectory, getLineCount, getLines,
  getGitRoot, matchesPattern, shouldExclude,
} from './filesystem-path.js';

export function readFile(p: string): string {
  try { return fs.readFileSync(resolvePath(p), 'utf-8'); }
  catch (e) {
    throw new AnalysisErrorClass(
      `Failed to read file: ${p}`, (e as Error).message
    );
  }
}
...
export function writeFile(p: string, content: string): void {
  try {
    ...
    throw new AnalysisErrorClass(
      `Failed to write file: ${p}`, (e as Error).message
    );
  }
}

export function readJsonFile<T>(p: string): T {
  try { return JSON.parse(readFile(p)) as T; }
  catch (e) {
    throw new AnalysisErrorClass(
      `Failed to parse JSON: ${p}`, (e as Error).message
    );
  }
}
...
export function writeJsonFile<T>(p: string, data: T, pretty = true): void {
  try {
    ...
    throw new AnalysisErrorClass(
      `Failed to write JSON: ${p}`, (e as Error).message
    );
  }
}

export function listFilesRecursive(
  dirPath: string, extensions?: string[], excludePatterns: string[] = []
): string[] {
  ...
  try {
    ...
    logger.warn(
      `Failed to list: ${dirPath}`, { error: (e as Error).message }
    );
  }
  return results;
}
...
export function ensureDirectory(p: string): void {
  try {
    ...
    throw new AnalysisErrorClass(
      `Failed to create dir: ${p}`, (e as Error).message
    );
  }
}
...
export function deletePathSync(p: string): void {
  try {
    ...
    throw new AnalysisErrorClass(
      `Failed to delete: ${p}`, (e as Error).message
    );
  }
}
...
export function getChangedFiles(since?: string): string[] {
  try {
    ...
    return (execSync(cmd, { cwd: process.cwd(), encoding: 'utf-8' }) as string)
      ...
      .map((l: string) => l.trim());
  } catch { logger.warn('Failed to get changed files from git'); return []; }
}
