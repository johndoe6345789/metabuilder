...
export function ensureHistoryDirectory() {
    try {
        ...
        logger.warn(`Failed to create history directory: ${e.message}`);
    }
}
...
export function readHistoryFile() {
    try {
        ...
        logger.warn(`Failed to load trend history: ${e.message}`);
        return emptyHistory();
    }
}
export function writeHistoryFile(history) {
...
export function deleteHistoryFile() {
    try {
        ensureHistoryDirectory();
        const p = getHistoryPath();
        if (fs.existsSync(p)) {
        ...
        logger.warn(`Failed to clear history: ${e.message}`);
    }
}
