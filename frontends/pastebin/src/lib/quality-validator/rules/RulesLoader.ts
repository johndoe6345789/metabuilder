...
export class RulesLoader {
  ...
  rulesFileExists(): boolean { return existsSync(this.getFilePath()); }
  getRulesFilePath(): string { return this.getFilePath(); }

  async listRules(): Promise<void> {
    const rules = await this.loadRulesFromFile();
    if (rules.length === 0) { logger.info('No custom rules defined'); return; }
