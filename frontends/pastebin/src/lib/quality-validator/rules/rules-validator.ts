...
function validateNaming(
  ...
  if (!n.pattern) {
    ...
    try { new RegExp(n.pattern); } catch (e) {
      errors.push(
...
function validateStructure(
  ...
  const valid = ['maxFileSize', 'missingExports', 'invalidDependency', 'orphanedFile'];
  if (!s.check) {
...
export function validateRulesConfig(
  ...
  for (let i = 0; i < rules.length; i++) {
    ...
    if (!rule.id) { errors.push(`${prefix}: Missing required field 'id'`); continue; }
    ...
    if (!ruleBase.message) errors.push(`${prefix} (${rule.id}): Missing 'message'`);

    switch (rule.type) {
      case 'pattern':    validatePattern(rule, errors, prefix);    break;
      case 'complexity': validateComplexity(rule, errors, prefix); break;
      case 'naming':     validateNaming(rule, errors, prefix);     break;
      case 'structure':  validateStructure(rule, errors, prefix);  break;
    }
  }
