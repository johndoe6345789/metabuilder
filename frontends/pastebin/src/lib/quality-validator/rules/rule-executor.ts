...
function calcComplexity(content: string, type: string): { value: number } {
  switch (type) {
    case 'lines':
    ...
    case 'parameters': {
      ...
      while ((m = re.exec(content)) !== null) {
        counts.push(
          (m[1] || m[2] || '').split(',').filter(p => p.trim()).length
        );
      }
    ...
    case 'nesting': {
      ...
      for (const ch of content) {
        if ('({['.includes(ch)) max = Math.max(max, ++cur);
    ...
    case 'cyclomaticComplexity': {
      const decisions = content.match(
        /\b(if|else|case|catch|for|while|&&|\|\||switch)\b/g
      ) || [];
...
export async function executePatternRule(
  rule: PatternRule,
  sourceFiles: string[],
  ...
  try {
    ...
    for (const file of sourceFiles) {
      ...
      try {
        ...
        logger.debug(`Failed to read file ${file}: ${(err as Error).message}`);
      }
    }
  } catch (err) {
    logger.error(`Pattern rule ${rule.id} error: ${(err as Error).message}`);
  }
}

export async function executeComplexityRule(
  rule: ComplexityRule,
  sourceFiles: string[],
  violations: RuleViolation[],
): Promise<void> {
    ...
    for (const file of sourceFiles) {
      ...
      try {
        ...
        if (metrics.value > rule.threshold) {
          violations.push({
            ruleId: rule.id, ruleName: rule.message, file,
            message:
              `${rule.message}: ${metrics.value} (threshold: ${rule.threshold})`,
            severity: rule.severity,
        ...
        logger.debug(
          `Complexity check failed for ${file}: ${(err as Error).message}`
        );
      }
    }
  } catch (err) {
    logger.error(
      `Complexity rule ${rule.id} error: ${(err as Error).message}`
    );
  }
}
