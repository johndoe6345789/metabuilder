...
export {
  executeNamingRule,
  executeStructureRule,
} from './rule-executor-naming.js'
...
function calcComplexity(content: string, type: string): { value: number } {
  switch (type) {
    ...
    case 'cyclomaticComplexity': {
      const decisions =
        content.match(/\b(if|else|case|catch|for|while|&&|\|\||switch)\b/g) ||
        []
...
export async function executeComplexityRule(
  ...
  try {
    ...
    logger.error(`Complexity rule ${rule.id} error: ${(err as Error).message}`)
  }
}
