...
export function buildRulesResult(violations, rulesApplied, startTime, config) {
    const bySev = {
    ...
    return {
        ...
        scoreAdjustment: Math.max(-bySev.critical * 2 - bySev.warning - bySev.info * 0.5, -10),
        executionTime: performance.now() - startTime,
        rulesApplied,
    };
}
export function emptyRulesResult(startTime) {
    return {
        violations: [],
        totalViolations: 0,
...
export function mapRuleSeverity(s) {
    const map = {
        critical: 'critical',
        warning: 'high',
        info: 'low',
...
export function checkRulesConfig(rules) {
    const errors = [];
    if (!rules.length) {
