 ...
 */
export function calcCodeQualityScore(metrics) {
    if (!metrics)
        return 50;
...
function calcComplexityScore(complexity) {
    if (!complexity)
        return 50;
    const dist = complexity.distribution;
    if (!dist)
        return 50;
...
function calcDuplicationScore(duplication) {
    if (!duplication)
        return 50;
    const pct = duplication.percent;
    if (pct === undefined)
        return 50;
    if (pct < 3)
        return 100;
...
function calcLintingScore(linting) {
    if (!linting)
        return 50;
...
export function calcTestCoverageScore(metrics) {
    if (!metrics)
        return 30;
    const overall = metrics.overall;
    const avg = (overall.lines.percentage +
...
export function calcArchitectureScore(metrics) {
    if (!metrics)
        return 50;
...
export function calcSecurityScore(metrics) {
    if (!metrics)
        return 50;
    let score = 100;
    if (metrics.vulnerabilities?.length) {
        const crit = metrics.vulnerabilities.filter(v => v.severity === 'critical').length;
        const high = metrics.vulnerabilities.filter(v => v.severity === 'high').length;
        score -= crit * 25 + high * 10;
    }
    if (metrics.codePatterns?.length) {
        const crit = metrics.codePatterns.filter(p => p.severity === 'critical').length;
