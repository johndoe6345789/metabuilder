...
export function generateRecommendations(codeQuality, testCoverage, architecture, security, _findings) {
    const recs = [];
    if (codeQuality) {
        if (codeQuality.complexity.distribution.critical > 0) {
            recs.push({
                ...
                remediation: `Refactor ${codeQuality.complexity.distribution.critical}` +
    ...
    if (testCoverage) {
        if (testCoverage.overall.lines.percentage < 80) {
            recs.push({
                ...
                remediation: `Increase coverage from ` +
        ...
        if (testCoverage.effectiveness.effectivenessScore < 70) {
            recs.push({
                ...
                remediation: 'Improve tests: more assertions, fewer mocks, better isolation',
                estimatedEffort: 'medium',
    ...
    if (architecture) {
        if (architecture.dependencies.circularDependencies.length > 0) {
            ...
            recs.push({
                ...
                remediation: `Resolve ${cnt} circular dependencies by restructuring modules`,
                estimatedEffort: 'medium',
    ...
    if (security) {
        const critVulns = security.vulnerabilities.filter(v => v.severity === 'critical').length;
        if (critVulns > 0) {
            recs.push({
                priority: 'critical',
                category: 'security',
                issue: 'Critical vulnerabilities',
                remediation: `Address ${critVulns} critical ` +
    ...
    recs.sort((a, b) => (PRIORITY_ORDER[a.priority] ?? 999) -
