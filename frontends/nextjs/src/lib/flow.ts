// Bezier-path helper for the workflow-editor ConnectionLine. Ported from
// workflowui (frontends/workflowui/src/lib/flow.ts) so the shared
// @/workflow-editor components resolve their `@/lib/flow` import here too.
...
export function getBezierPath({
  ...
  const offset = getControlOffset(Math.max(dist, 100), curvature)
  ...
  const path =
    `M${sourceX},${sourceY} C${sourceControlX},${sourceControlY} ` +
    `${targetControlX},${targetControlY} ${targetX},${targetY}`
