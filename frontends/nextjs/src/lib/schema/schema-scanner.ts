...
export function scanAllPackages(
  ...
  for (const entry of entries) {
    ...
    const packageId =
      typeof metadata.packageId === 'string' && metadata.packageId.length > 0
        ? metadata.packageId
        : entry.name
    ...
    if (!Array.isArray(rawEntities)) {
      result.errors.push(
        `Schema entities missing or invalid for package ${packageId}`
      )
      continue
    }
    ...
    const invalidEntities = rawEntities.filter(
