...
const BLANK: Omit<StreamApp, 'id'> = {
  name: '', url: '', bgColor: '#222222', fgColor: '#ffffff', embedMode: 'newtab', sortOrder: 0,
}
...
export function AppsSettingsModal({ apps, onClose, onCreate, onUpdate, onDelete }: Props) {
          ...
          <div className={s.list}>
            {apps.map(app => (
              <div key={app.id} className={s.row}>
                <span className={s.swatch} style={{ background: app.bgColor, color: app.fgColor }}>
                ...
                <select
                  ...
                  onChange={(e) => { void handleEmbedModeChange(app, e.target.value as EmbedMode) }}
                >
            ...
            <div className={s.addGrid}>
              ...
              <label className={s.colorField}>
                ...
                <input
                  type="color"
                  value={draft.fgColor}
                  onChange={(e) => { setDraft({ ...draft, fgColor: e.target.value }) }}
                />
              </label>
              <select
                className={s.select}
                value={draft.embedMode}
                onChange={(e) => { setDraft({ ...draft, embedMode: e.target.value as EmbedMode }) }}
              >
                <option value="newtab">New tab</option>
                <option value="iframe">Embed (iframe)</option>
              </select>
              <button className={s.addBtn} disabled={busy === '__new__'} onClick={() => { void handleAdd() }}>
                {busy === '__new__' ? 'Adding…' : '+ Add'}
              </button>
            </div>
            {formError !== null && <p className={s.formError}>{formError}</p>}
            <p className={s.hint}>
              Most streaming services block iframe embedding on purpose (X-Frame-Options/CSP) —
              &quot;Embed&quot; will only actually show content for services that allow it.
            </p>
          </div>
        </div>
      </div>
