...
type Context = { vault: Controller; view: typeof vaultView } & Record<string, unknown>
...
function evaluate(binding: string | boolean | VaultBinding, context: Context): unknown {
  ...
  if ('$eq' in binding) {
    return evaluate(binding.$eq[0], context) === evaluate(binding.$eq[1], context)
  }
  ...
  if ('$classes' in binding) {
    return binding.$classes.flatMap(value => {
      if (typeof value === 'string') return s[value] ?? []
      return evaluate(value.when, context) === true ? s[value.name] ?? [] : []
    }).join(' ')
  }
  const reference = typeof binding.$event === 'string'
    ? binding.$event : evaluate(binding.$event, context)
  if (typeof reference !== 'string') throw new Error('Event reference must resolve to a string')
  ...
  return binding.$value === 'target.value'
    ? (event: { target: { value: string } }) => handler(...args, event.target.value)
    : () => handler(...args)
}

function nodeProps(node: VaultTreeNode, context: Context): Record<string, unknown> {
  const props = Object.fromEntries(
    Object.entries(node.props ?? {}).map(([key, value]) => [key, evaluate(value, context)])
  )
  if (node.className !== undefined) {
    props.className = typeof node.className === 'string'
      ? s[node.className] : evaluate(node.className, context)
  }
  return props
}
...
function VaultNode({ node, context }: { node: VaultTreeNode; context: Context }) {
  const { vault } = context
  if (node.when !== undefined && !isTruthy(evaluate(node.when, context))) return null
  if (node.component === 'Repeat') {
    const source = node.source === undefined ? [] : evaluate(node.source, context)
    if (!Array.isArray(source) || node.item === undefined) return null
    return source.map((item, index) => {
      const scoped = { ...context, [node.item!]: item, index }
      const key = node.key === undefined ? index : templateValue(evaluate(node.key, scoped))
      return <span key={key} className={s.repeatItem}>{childrenOf(node, scoped)}</span>
    })
  }
  const children = childrenOf(node, context)
  if (['div', 'header', 'button', 'strong', 'span'].includes(node.component)) {
    const className = node.className === undefined ? undefined
      : typeof node.className === 'string' ? s[node.className]
        : evaluate(node.className, context)
    const text = node.text === undefined ? undefined : evaluate(node.text, context)
  ...
  switch (node.component) {
    case 'Page': return <div className={s.page}>{children}</div>
    case 'Loading': return <Typography variant="body2">{vaultView.loadingLabel}</Typography>
    case 'Notice': return vault.notice === null ? null
      : <Alert severity={vault.notice.kind} className={s.alert}>{vault.notice.message}</Alert>
    case 'SplitLayout': return <div className={s.splitLayout}>{children}</div>
    case 'ListPanel': return <Paper className={s.listPanel}>{children}</Paper>
    case 'EditorPanel': return <Paper className={s.editorPanel}>{children}</Paper>
  }
  return null
}
...
export function VaultTreeRenderer() {
  ...
  const view = vault.authLoading ? 'loading' : vault.authenticated ? 'unlocked' : 'locked'
  return <VaultNode node={vaultView.views[view]} context={{ vault, view: vaultView }} />
}
