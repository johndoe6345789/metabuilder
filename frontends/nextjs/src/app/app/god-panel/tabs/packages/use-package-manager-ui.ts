...
export function usePackageManagerUi() {
  ...
  const [draft, setDraft] = useState<EditDraft>({ name: '', version: '', description: '' })
  const [flash, setFlash] = useState<string | null>(null)

  const beginEdit = useCallback((id: string, d: EditDraft) => {
    setEditingId(id); setDraft(d)
  }, [])
  ...
  return {
    newName, setNewName, showArchived, setShowArchived,
    editingId, draft, beginEdit, cancelEdit, patchDraft,
    flash, setFlash,
  }
}
