    ...
    <div className={s.fields}>
        ...
        <div className={s.chips}>
          {LEVELS.map(lvl => (
            <Chip
              ...
              color={
                form.level === lvl.value ? 'primary' : 'default'
              }
              variant={
                form.level === lvl.value ? 'filled' : 'outlined'
              }
              size="small"
              title={lvl.desc}
            />
      ...
      <div className={s.section}>
        ...
        <div className={s.chips}>
          <Chip
            label="Public"
            onClick={() => { onChange('requiresAuth', false) }}
            color={form.requiresAuth !== true ? 'primary' : 'default'}
            variant={
              form.requiresAuth !== true ? 'filled' : 'outlined'
            }
            size="small"
          />
          <Chip
            label="Requires login"
            onClick={() => { onChange('requiresAuth', true) }}
            color={form.requiresAuth === true ? 'primary' : 'default'}
            variant={
              form.requiresAuth === true ? 'filled' : 'outlined'
            }
          ...
          <Chip
            ...
            variant={
              form.isActive !== false ? 'filled' : 'outlined'
            }
            size="small"
          />
          <Chip
      ...
      <div className={s.fieldGroup}>
        ...
        <Textarea
          value={treeStr}
          onChange={e => { onChange('componentTree', e.target.value) }}
          rows={10}
