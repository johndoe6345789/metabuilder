  ...
  return (
    <div className={s.root}>
      <div className={s.bar}>
        ...
        {ran > 0 && (
          <span className={s.summary}>{passed}/{ran} passing</span>
        )}
        <span className={s.spacer} />
        <Button variant="outlined" size="small" onClick={t.create}>+ New test</Button>
        <Button variant="contained" size="small" disabled={t.running}
          onClick={() => { void t.runAll() }}>
          ...
          return (
            <div key={c.id} className={s.case}>
              <div className={s.caseHead}>
                <TextField size="small" label="Test name" value={c.name}
                  onChange={(e) => { t.update(c.id, { name: e.target.value }) }} />
                <span className={`${s.badge} ${b.cls ? s[b.cls] : ''}`}>{b.label}</span>
                <button className={s.del} onClick={() => { t.remove(c.id) }}>✕</button>
              </div>
              <div className={s.io}>
                <div className={s.field}>
                  <label>Input (JSON)</label>
                  <textarea className={s.code} value={c.input}
                    onChange={(e) => { t.update(c.id, { input: e.target.value }) }} />
                </div>
                <div className={s.field}>
                  <label>Expected (subset)</label>
                  <textarea className={s.code} value={c.expected}
                    onChange={(e) => { t.update(c.id, { expected: e.target.value }) }} />
                </div>
              </div>
