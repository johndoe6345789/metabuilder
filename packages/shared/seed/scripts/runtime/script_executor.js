/**
 * JSON Script Runtime Executor (JavaScript/TypeScript)
 * Executes script.json definitions at runtime without code generation
 *
 * Port of script_executor.lua to JavaScript
 */

/**
 * Reference resolver - resolves $ref:path.to.value
 * @param {any} ref - The reference value or string
 ...
 * @param {any} expr - Expression to evaluate
 * @param {Object} context - Execution context
...
function evalExpression(expr, context) {
  ...
  // Literal values
  if (exprType === 'string_literal') {
    return expr.value;
  }
  if (exprType === 'number_literal') {
    return expr.value;
  }
  if (exprType === 'boolean_literal') {
    return expr.value;
  }
  if (exprType === 'null_literal') {
    return null;
  }
  ...
  if (exprType === 'logical_expression') {
    ...
    if (op === '??') {
      const left = evalExpression(expr.left, context);
      if (left !== null && left !== undefined) return left; // Null coalescing
      return evalExpression(expr.right, context);
    }
  }
...
/**
 * Statement executor
 * @param {Object} stmt - Statement to execute
 * @param {Object} context - Execution context
...
function executeStatement(stmt, context) {
  ...
  if (stmtType === 'assignment') {
    ...
    // Handle $ref: target for complex assignments
 ...
 * @param {Object} scriptJson - Parsed script.json object
 ...
 * @param {string} jsonPath - Path to script.json file
 * @param {string} functionName - Name of function to execute
 * @param {Array} args - Arguments to pass to function
 * @returns {Promise<any>} Function result
 */
async function loadAndExecute(jsonPath, functionName, args = []) {
...
/**
 * Synchronous version of loadAndExecute for Node.js
 * @param {string} jsonPath - Path to script.json file
 * @param {string} functionName - Name of function to execute
 * @param {Array} args - Arguments to pass to function
 * @returns {any} Function result
