...
export class DBALReadExecutor implements INodeExecutor {
  ...
  async execute(
    ...
    try {
      ...
      const resolvedFilter = filter ? interpolateTemplate(filter, { context, state, json: context.triggerData }) : {};
      ...
      switch (operation || 'read') {
        case 'read':
          result = await db[entity.toLowerCase()].list({
        ...
        case 'validate':
          // Validation operation - check data against rules
          result = this._validateData(context.triggerData, node.parameters.rules || {});
          break;

        case 'aggregate':
          // Aggregation operation
          result = await db[entity.toLowerCase()].aggregate({
            filter: resolvedFilter,
   ...
   */
  private _validateData(data: any, rules: Record<string, string>): { valid: boolean; errors: Record<string, string> } {
