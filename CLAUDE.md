...
| An export file with no owner, restored wholesale | The Deploy tab's project bundle carried `{kind, version, god, idb}` and no record of whose editor it came from, and the import called `rehydrate(parsed.god)` on whatever it was handed. The god slice holds the page tree, the CSS, the workflows and the **SMTP host, username and password** -- so one community's export opened in another founder's browser silently became that founder's own drafts, ready to be published into their site. The tenant marker that guards this same slice against a browser-level tenant switch lives in `localStorage`, not in the file, so nothing downstream could tell either. The bundle records its tenant now and the import refuses another community's, naming both. A pre-v3 bundle has no tenant and is still accepted -- it was written before there was one -- but it is read as belonging to nobody rather than to whoever opened it. **A file that restores state is an import path into your data; it needs the same "whose is this" answer as the network does.** |
| An IDE with a file tree of files that did not exist | Nerd Mode listed `src/workflow.json`, `src/config.json` and `src/styles.css` -- names read from a localStorage key nothing writes -- and `openFileNode` set `content: ''`, so whatever you clicked the editor stayed blank. The panel already holds the real things those names describe, so the explorer now lists this community's page tree, stylesheet and open workflow, with their actual content. |
| A read-only pane that looks editable | Monaco was mounted with no `onChange` and no save, so a founder could type into it and watch the change vanish on the next click. It is explicitly `readOnly` now: these artefacts have real editors in the Components, Styles and Workflows tabs, and hand-editing the JSON of a live page tree is a good way to lose one. |
| An editor fetched from a public CDN at runtime | `@monaco-editor/react` loads the editor itself from `cdn.jsdelivr.net` and `monaco-editor` is **not** a dependency here -- confirmed by watching 14 requests to it fail on a restricted network, leaving a blank box with nothing to say why. This product ships as a Docker stack that may never have outbound internet. The pane now passes the file as plain text in the `loading` slot, so a restricted or offline install still shows the file rather than nothing; bundling `monaco-editor` locally would remove the dependency outright and is the fuller fix. |
| The E2E harness pointed a Server Component at the wrong port | `playwright.config.ts` never set `METABUILDER_INTERNAL_URL`, so it defaulted to port **3000** while the tests run the app on 3004 -- every Server Component call to this app's own `/api/v1` reached nothing under test. Invisible for as long as those pages answered "Authentication required" to everyone anyway. Set now, with `e2e/entity-admin.spec.ts` covering the surface: verified by reverting the cookie fix and watching two of its four specs go red. **`.env.example` names this variable and says exactly what breaks without it; the test harness is an environment too.** |
| A live player that attached hls.js and listened to nothing | hls.js does **not** recover by itself -- its documented contract is that the application handles `ERROR`, calling `startLoad()` after a fatal network error and `recoverMediaError()` after a fatal media one. `VideoPlayer` registered no handlers at all, so one segment 404 as the live window rolled (routine) or one decode hiccup stopped playback permanently, silently, with a black frame. This is a stream stopping "at the wrong time" and starting and stopping "randomly" -- reported from real use, not found by a test. Recovery is bounded (4 network, 2 media) and then says which of the two happened, because retrying for ever against a channel that has actually ended is a spinner with no end. Both TV and the retro sessions play through this component. |
| "Live TV was not live" | `liveMaxLatencyDurationCount` is **unset** in hls.js by default, and unset means it never seeks forward -- so after any stall, pause or spell in a background tab, playback carries on from wherever it was, arbitrarily far behind the edge, for the rest of the session. Configured now, plus a `waiting`/`playing` catch-up that snaps to `hls.liveSyncPosition` when the gap passes 12s. **A live player needs a policy for falling behind; the library's default is to stay behind.** |
| The same swallowed failure in both media players | `handleWatch` and `handleListen` called a `start()` that throws on a refusal or a missing stream URL, inside a `try/finally` with no `catch`: the spinner cleared, no player appeared, and the rejection went unhandled. From the viewer's side the button worked sometimes and did nothing other times. Both say why now. The live TV trigger had a race with the same effect -- it marked the "watch this" nonce as spent **before** looking the channel up, so a "Watch now" pressed while the list was still loading consumed the trigger and started nothing, and the guard then blocked it for ever. |
| A dropped connection is how a live audio stream ends | An `<audio>` playing a live stream fires `error`, or `ended` -- which for live means the connection dropped, since a live stream has no end -- and nothing listened to either, so the play button went back to paused and the listener was left thinking the station stopped by itself. `useLiveAudio` reconnects with backoff, bounded, and only for live: reloading a recorded track would restart it from the beginning, which is not a repair. |
| Every OS key repeat posted as a fresh button press | Holding a direction fires `keydown` at the repeat rate -- tens of events a second -- and the retro launcher posted each one to the media daemon. A held d-pad was a flood of identical requests, and lag in an emulator is the difference between playable and not. Only transitions are sent now (`held-buttons.ts`), which is also what makes the release reliable: a key held through an alt-tab never gets its `keyup`, so the button stayed down and the character kept running into the wall. Blur, tab-hide and session end all let go of everything still held. |
| A touch pad button that released on `pointerup` alone | `pointerup` lands on whatever is under the pointer, so sliding a thumb off the d-pad -- the ordinary way anyone plays -- released nothing and left the button down for ever. The button captures the pointer now and releases on cancel and lost-capture too; `touch-action: none` stops the page scrolling and zooming under the thumb instead of playing. |
| A retro game server with no support for a game controller | `navigator.getGamepads()` has been in browsers for years and the launcher used none of it -- an on-screen pad and a keyboard were the only ways to play. A gamepad reports no events for its buttons, it has to be polled, so it is read once a frame while a session runs and only changes are sent. Face buttons follow the W3C standard mapping by position with the SNES naming those cores expect (south is B, east is A), the left stick doubles as the d-pad for the many pads that report no d-pad at all, and unplugging mid-game releases whatever it was holding. |
| Telling the daemon you are watching and never telling it you left | `start` is how this client says "I am watching this channel", and changing channel called `start` again without ever calling `stop` -- so the first channel kept running with a viewer that had gone. The app already treats leaving as a reason to stop (that is what Back does), so a switch now stops the old one first, best effort, and starts the new one regardless. The same in radio. Watch the `useRef` here: `handleWatch` reads what is playing before its own `setState` has landed, so state alone would stop the wrong channel or none. |
| A test that mutated the shared mock and left it that way | The live-TV mock is module-level, and a new test set `tv.channels` to its own two-channel list without restoring it -- so whatever ran next inherited it, and which tests those are depends on the order vitest happens to pick. One run in six failed. Restored in an `afterEach` now. **A module-level mock is shared state; the rule for it is the rule for the spies two rows below -- put it back, in a hook that runs whatever the test did.** |
| A ROM address the daemon had to reject on the player's behalf | The retro form accepted any non-empty string, so a typo, a bare filename or a `file://` path went to the media daemon and came back as an error the player had no way to connect to what they had typed. Checked in the form now, before it is sent. **It is a courtesy and not a control**: anything can POST to the daemon directly, so refusing a ROM address that points somewhere it should not go -- `file://`, its own loopback, a cloud metadata endpoint -- has to happen on the daemon's side, which is a sibling repo. Worth confirming it does. |
