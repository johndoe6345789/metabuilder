...
| Browsing the local stack on `http://0.0.0.0:8080` | Sign In dies with `Cannot read properties of undefined (reading 'digest')` and **no** network request. `0.0.0.0` is not a [secure context](https://developer.mozilla.org/en-US/docs/Web/Security/Secure_Contexts) — Chrome trusts `localhost` and `127.0.0.1`, not `0.0.0.0` — so `crypto.subtle` is `undefined` and PKCE's `sha256Base64Url` (`redux/dbal-sso/src/core/pkce.ts`) throws. Note `crypto.getRandomValues` on the line before works fine (it isn't secure-context gated), so the failure lands mid-function and reads like a bug in the SSO library. Always use `http://localhost:8080/app` |
