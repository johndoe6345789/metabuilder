...
| Browsing the local stack on `http://0.0.0.0:8080` | Sign In fails with **no** network request. `0.0.0.0` is not a [secure context](https://developer.mozilla.org/en-US/docs/Web/Security/Secure_Contexts) — Chrome trusts `localhost` and `127.0.0.1`, not `0.0.0.0` — so `crypto.subtle` is `undefined` and PKCE's `sha256Base64Url` (`redux/dbal-sso/src/core/pkce.ts`) cannot hash the verifier. Note `crypto.getRandomValues` on the line before works fine (it isn't secure-context gated), so the failure lands mid-function. Always use `http://localhost:8080/app`. Since redux `c1c8d9a` that function guards on `!crypto.subtle` and throws a message naming the origin and the fix; **before** that commit it threw a bare `Cannot read properties of undefined (reading 'digest')`, which reads like a bug in the SSO library rather than a bad address bar. An image built from a `workspace.json` redux pin older than `c1c8d9a` still shows the old message — check the pin, not just the browser. A Tailscale MagicDNS hostname over plain `http://` is not a secure context either — front it with real HTTPS (`tailscale serve --bg 8080`) and register the origin in `deploy/oidc/clients.json` |
