...
import { Pencil, Star, Plus, ArrowUp, ArrowDown } from '@/fakemui'
...
const MOCK_WORKFLOWS: WorkflowCard[] = [
  {
    id: 'workflow-1',
    name: 'User Authentication Flow',
    description: 'OAuth 2.0 login workflow',
    nodeCount: 8,
    status: 'published',
  ...
  useEffect(() => {
    if (projectId) {
      ...
      try {
        ...
        setWorkflowCards(MOCK_WORKFLOWS)
        setIsLoading(false)
      } catch (err) {
        error('Failed to load project canvas')
        setIsLoading(false)
      } finally {
        setLoading(false)
      }
    }
  }, [projectId, setLoading, error])
      ...
      <AppBar position="sticky" elevation={1}>
        <Toolbar>
          <Box sx={{ flex: 1 }}>
            ...
            <Typography variant="body2" color="textSecondary">
              {workflowCards.length} workflows • Project ID: {projectId}
            </Typography>
          </Box>
              ...
              <Grid item xs={12} sm={6} md={4} key={workflow.id}>
                ...
                >
                  ...
                  <CardContent sx={{ pb: 1 }}>
                    ...
                    >
                      {Array.from({ length: Math.min(workflow.nodeCount, 6) }).map(
                        (_, i) => (
                          <Box
                            key={i}
                            sx={{
