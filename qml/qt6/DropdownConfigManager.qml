...
Rectangle {
    ...
    function loadDropdowns() {
        dbal.execute("core/dropdown-configs", {}, function(result, error) {
            if (!error && result && result.items && result.items.length > 0) {
                dropdowns = Crud.parseDbalItems(result.items);
    ...
    function saveDropdown(index) {
        ...
        var dd = dropdowns[index], data = Crud.toSaveData(dd);
    ...
    Component.onCompleted: { Crud.loadJson("config/dropdown-defaults.json", function(data) { dropdowns = data; }); loadDropdowns(); }
    ...
    ColumnLayout {
        ...
        RowLayout {
            ...
            DropdownSidebar {
                ...
                dropdowns: root.dropdowns; selectedIndex: root.selectedIndex
                onItemClicked: function(idx) { root.selectedIndex = idx; }
                onAddClicked: addDialogOpen = true
            }

            CDropdownEditorPanel {
                dropdown: selectedDropdown(); selectedIndex: root.selectedIndex
                onDeleteClicked: deleteDialogOpen = true
                onFieldChanged: function(f, v) { applyUpdate(Crud.updateField(dropdowns, selectedIndex, f, v)); }
                onAddOptionClicked: applyUpdate(Crud.addOption(dropdowns, selectedIndex))
                onRemoveOptionClicked: function(i) { applyUpdate(Crud.removeOption(dropdowns, selectedIndex, i)); }
                onMoveOptionClicked: function(i, d) { applyUpdate(Crud.moveOption(dropdowns, selectedIndex, i, d)); }
                onOptionFieldChanged: function(i, f, v) { applyUpdate(Crud.updateOptionField(dropdowns, selectedIndex, i, f, v)); }
            }
        }
    }
    ...
    CAddDropdownDialog {
        ...
        onCreateRequested: function(name, desc) {
            var newDd = Crud.createDropdown(name, desc);
            if (useLiveData) dbal.execute("core/dropdown-configs/create", { data: newDd }, function(r, e) { if (!e) loadDropdowns(); });
            dropdowns = Crud.addDropdown(dropdowns, newDd);
    ...
    CDeleteDropdownDialog {
        visible: deleteDialogOpen; dropdown: selectedDropdown()
        onConfirmed: {
            if (useLiveData && dropdowns[selectedIndex] && dropdowns[selectedIndex].id)
                dbal.execute("core/dropdown-configs/delete", { id: dropdowns[selectedIndex].id }, function(r, e) { if (!e) loadDropdowns(); });
            dropdowns = Crud.removeDropdown(dropdowns, selectedIndex);
            selectedIndex = dropdowns.length > 0 ? Math.min(selectedIndex, dropdowns.length - 1) : -1; deleteDialogOpen = false;
        }
