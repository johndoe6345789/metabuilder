...
function httpRequest(baseUrl, method, endpoint, body, callback) {
    var xhr = new XMLHttpRequest();
    xhr.onreadystatechange = function() {
...
function toggleRadio(radioChannels, index) {
    ...
    if (ch.status === "live") {
        ch.status = "offline"; ch.listeners = 0; ch.currentTrack = "---";
    } else {
        ch.status = "live"; ch.listeners = Math.floor(Math.random() * 200) + 10; ch.currentTrack = ch.playlist[0];
    }
    updated[index] = ch;
    return updated;
}
...
function toggleTv(tvChannels, index) {
    ...
    if (ch.status === "broadcasting") {
        ch.status = "offline"; ch.viewers = 0; ch.uptime = "0m";
    } else {
        ch.status = "broadcasting"; ch.viewers = Math.floor(Math.random() * 500) + 20; ch.uptime = "0m";
    }
    updated[index] = ch;
    return updated;
}
