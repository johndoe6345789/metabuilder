...
Rectangle {
    ...
    function addComment(text) {
        ...
        commentsModel.insert(0, {
            commentId: commentsModel.count + 1, username: appWindow.currentUser,
            initials: appWindow.currentUser.substring(0, 2).toUpperCase(),
            timestamp: "Just now", body: text, likes: 0, liked: false
        })
        DBAL.postComment(dbal, text, appWindow.currentUser)
    }
    ...
    Component.onCompleted: {
        ...
        dbal.ping(function(success) { if (success) DBAL.loadComments(dbal, commentsModel) })
    }
    ...
    ScrollView {
        ...
        ColumnLayout {
            ...
            FlexRow {
                ...
                CTabBar { tabs: [{ label: "Newest" }, { label: "Oldest" }, { label: "Most Liked" }]; onCurrentIndexChanged: sortMode = currentIndex }
            }
            ...
            Repeater {
                ...
                delegate: Meta.CCommentCard {
                    comment: ({ username: model.username, initials: model.initials, timestamp: model.timestamp, body: model.body, likes: model.likes, liked: model.liked, canDelete: canDelete(model.username) })
                    ...
                    onLiked: {
                        ...
                        DBAL.likeComment(dbal, model.commentId, newLikes)
                    }
                    onDeleted: { DBAL.deleteComment(dbal, model.commentId); commentsModel.remove(index) }
                }
            }
