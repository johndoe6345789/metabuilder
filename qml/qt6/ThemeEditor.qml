...
Rectangle {
    ...
    property var themeDefinitions: TELogic.loadJson(Qt.resolvedUrl("qmllib/MetaBuilder/data/theme-definitions.json"))

    ScrollView {
        anchors.fill: parent; anchors.margins: 24; clip: true; contentWidth: availableWidth
        ColumnLayout {
            ...
            ThemeColorTokens {
                customPrimary: root.customPrimary; customBackground: root.customBackground; customSurface: root.customSurface; customPaper: root.customPaper
                customText: root.customText; customTextSecondary: root.customTextSecondary; customBorder: root.customBorder
                customError: root.customError; customWarning: root.customWarning; customSuccess: root.customSuccess; customInfo: root.customInfo
                onColorChanged: function(token, value) { TELogic.applyColorChange(root, token, value); hasUnsavedChanges = true }
            }

            ThemeTypography { fontFamily: root.fontFamily; baseFontSize: root.baseFontSize; onFontFamilyChanged: function(f) { root.fontFamily = f; hasUnsavedChanges = true }; onBaseFontSizeChanged: function(s) { root.baseFontSize = s; hasUnsavedChanges = true } }

            ThemeSpacingRadius {
                baseSpacing: root.baseSpacing; radiusSmall: root.radiusSmall; radiusMedium: root.radiusMedium; radiusLarge: root.radiusLarge
                onBaseSpacingEdited: function(v) { root.baseSpacing = v; hasUnsavedChanges = true }
                onRadiusSmallEdited: function(v) { root.radiusSmall = v; hasUnsavedChanges = true }
                onRadiusMediumEdited: function(v) { root.radiusMedium = v; hasUnsavedChanges = true }
                onRadiusLargeEdited: function(v) { root.radiusLarge = v; hasUnsavedChanges = true }
            }
