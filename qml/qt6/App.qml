...
ApplicationWindow {
    ...
    function login(u, p) { return Logic.login(appWindow, u, p) }
    function logout() { Logic.logout(appWindow, dbalProvider) }
    function viewIndex(v) { return Logic.viewIndex(appWindow) }
    function packageViewName(pkg) { return Logic.packageViewName(pkg) }
    ...
    header: CAppBar {
        ...
        RowLayout {
            ...
            CNavBar { currentView: appWindow.currentView; currentLevel: appWindow.currentLevel; onNavigate: function(view) { appWindow.currentView = view } }
            Item { Layout.fillWidth: true }
            Item { width: 4 }
            ...
            CUserMenu {
                visible: loggedIn; username: currentUser; level: currentLevel; role: currentRole; isDark: appWindow.isDark
                onNavigateTo: function(view) { appWindow.currentView = view }; onSignOut: logout()
            }
        }
    }
    ...
    RowLayout {
        ...
        CSidebar {
            ...
            packageViewName: appWindow.packageViewName; onNavigate: function(view) { appWindow.currentView = view }
        }
        Rectangle {
            ...
            StackLayout {
                ...
                SettingsView {}
                CommentsView {}
                Repeater { model: PackageLoader.navigablePackages(); delegate: PackageViewLoader { packageId: modelData.packageId } }
            }
        }
    }
    ...
    Settings {
        ...
        property alias windowWidth: appWindow.width; property alias windowHeight: appWindow.height
        property alias windowX: appWindow.x; property alias windowY: appWindow.y
        property alias theme: appWindow.currentTheme; property alias authToken: appWindow.authToken
    }

    onCurrentThemeChanged: { if (typeof Theme.setTheme === "function") Theme.setTheme(currentTheme) }
    Component.onCompleted: Logic.autoLogin(appWindow, dbalProvider)
