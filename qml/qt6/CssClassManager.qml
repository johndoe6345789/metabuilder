...
import "../MetaBuilder/CssClassLogic.js" as Logic
...
Rectangle {
    ...
    ColumnLayout {
        anchors.fill: parent; spacing: 16; anchors.margins: 20
        ...
        RowLayout {
            ...
            CssClassSidebar {
                ...
                cssClasses: root.cssClasses; selectedIndex: root.selectedClassIndex
                onItemClicked: function(idx) { root.selectedClassIndex = idx }
            }
            ColumnLayout {
                Layout.fillWidth: true; Layout.fillHeight: true; spacing: 16
                CssPropertyEditor {
                    Layout.fillWidth: true; Layout.fillHeight: true; visible: selectedClass() !== null
                    selectedClass: root.selectedClass(); propertySuggestions: root.propertySuggestions
                    onClassNameChanged: function(name) { Logic.setClassName(root, dbal, name) }
                    onDeleteClassClicked: showDeleteConfirm = true
                    onAddPropertyClicked: Logic.addPropertyToSelected(root, dbal)
                    onRemovePropertyClicked: function(idx) { Logic.removePropertyFromSelected(root, dbal, idx) }
                    onPropertyNameChanged: function(idx, name) { Logic.setPropertyName(root, dbal, idx, name) }
                    onPropertyValueChanged: function(idx, val) { Logic.setPropertyValue(root, dbal, idx, val) }
                }
                CssClassPreview {
                    Layout.fillWidth: true; Layout.preferredHeight: 160
                    visible: selectedClass() !== null; selectedClass: root.selectedClass()
                }
            }
        }
    }
    ...
    CDialog {
        ...
        ColumnLayout {
            ...
            FlexRow {
                ...
                CButton { text: "Create"; variant: "primary"; size: "sm"; enabled: newClassName.trim().length > 0; onClicked: { Logic.addClass(root, dbal, newClassName.trim()); showAddClassDialog = false } }
            }
        }
    }
    ...
    CDialog {
        ...
        ColumnLayout {
            ...
            FlexRow {
                ...
                CButton { text: "Delete"; variant: "danger"; size: "sm"; onClicked: { Logic.deleteSelectedClass(root, dbal); showDeleteConfirm = false } }
            }
        }
    }
}
