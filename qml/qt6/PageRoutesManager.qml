...
Rectangle {
    ...
    readonly property bool hasSel: selectedIndex >= 0 && selectedIndex < routes.length
    ...
    function reload() { DBAL.loadRoutes(dbal, function(r) { routes = r }) }
    function updateRoute(i, field, value) {
        routes = DBAL.updateRoute(routes, i, field, value)
        if (selectedIndex === i) { selectedIndex = -1; selectedIndex = i }
        if (useLiveData) DBAL.saveRoute(dbal, routes[i], function() { reload() })
    }
    function addRoute(p, t, l, ly) {
        if (useLiveData) DBAL.createRoute(dbal, { path: p, title: t, level: l, layout: ly, enabled: true, permissions: "authenticated" }, function() { reload() })
        else routes = DBAL.addRouteLocal(routes, p, t, l, ly)
    }
    function deleteRoute() {
        if (!hasSel) return
        if (useLiveData && routes[selectedIndex].id) DBAL.removeRoute(dbal, routes[selectedIndex].id, function() { reload() })
    ...
    onUseLiveDataChanged: if (useLiveData) reload()
        ...
        RowLayout {
            ...
            CCard {
                ...
                ColumnLayout {
                    ...
                    ListView {
                        Layout.fillWidth: true; Layout.fillHeight: true; model: routes; clip: true; spacing: 1
            ...
            CRouteEditPanel {
                Layout.preferredWidth: 340; Layout.fillHeight: true; visible: hasSel
                route: hasSel ? routes[selectedIndex] : null
            ...
            CCard {
                ...
                ColumnLayout {
                    ...
                    CText { variant: "body2"; text: "Select a route to edit its configuration."; Layout.fillWidth: true; horizontalAlignment: Text.AlignHCenter; wrapMode: Text.WordWrap }
                    Item { Layout.fillHeight: true }
                }
            }
        }
    ...
    CAddRouteDialog {
        visible: addDialogVisible; layoutOptions: root.layoutOptions; levelOptions: root.levelOptions
        onAddRoute: function(p, t, l, ly) { root.addRoute(p, t, l, ly) }
    }
    CDeleteConfirmDialog {
        visible: deleteDialogVisible; title: "Delete Route"
        itemName: hasSel ? routes[selectedIndex].path : ""
