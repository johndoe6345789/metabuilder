...
// DBAL persistence and local CRUD helpers for user management.
...
function filteredUsers(users, searchText, activeRoleFilter) {
...
function createUserLocally(users, nextUid, userData) {
...
function saveEditLocally(users, editIndex, userData) {
    var copy = users.slice()
...
function deleteLocally(users, deleteIndex) {
    var c = users.slice(); c.splice(deleteIndex, 1); return c
}
