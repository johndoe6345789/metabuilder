    ...
    ColumnLayout {
        ...
        RowLayout {
            ...
            CTextField {
                ...
                onTextChanged: { var val = parseInt(text); if (!isNaN(val) && val >= 0) radiusEditor.radiusSmallEdited(val) }
            }
            CTextField {
                Layout.preferredWidth: 100; label: "Medium (px)"
                placeholderText: "8"; text: radiusMedium.toString()
                onTextChanged: { var val = parseInt(text); if (!isNaN(val) && val >= 0) radiusEditor.radiusMediumEdited(val) }
            }
            CTextField {
                Layout.preferredWidth: 100; label: "Large (px)"
                placeholderText: "16"; text: radiusLarge.toString()
                onTextChanged: { var val = parseInt(text); if (!isNaN(val) && val >= 0) radiusEditor.radiusLargeEdited(val) }
            }
