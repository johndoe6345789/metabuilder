...
// Delete removes a key from a dictionary.
	...
	for i := 0; i < len(parts)-1; i++ {
		...
		if val, exists := current[part]; exists {
			if nested, ok := val.(map[string]interface{}); ok {
				...
				copied := copyDictForDelete(nested)
				current[part] = copied
				current = copied
			} else {
...
// copyDictForDelete creates a shallow copy of a dictionary.
func copyDictForDelete(d map[string]interface{}) map[string]interface{} {
